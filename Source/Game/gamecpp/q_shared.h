...
typedef struct mtexinfo_s
{
	...
	int						flags;//20
	int						numframes;//24
	struct mtexinfo_s		*next;		// animation chain28
	struct image_s			*image;//2C
	struct image_s			*origimage;//30
	// Required for server/client loading
	struct ctextureinfo_s	*textureinfo;//34
	struct ctextureinfo_s	*origtextureinfo;//38
	int						value;//3C
	int						Damage;//40
	int						DamageLevel;//44
} mtexinfo_t;
...
typedef struct
{
	...
	short		origin[3];		// 12.3 // 4
	short		velocity[3];	// 12.3 // A
	byte		pm_flags;		// ducked, jump_held, etc // 10
	byte		pm_time;		// each unit = 8 ms // 11
	short		gravity; // 12
	short		delta_angles[3];// add to command angles to get view direction // 14
								// changed by spawns, rotating objects, and teleporters
	byte		moveScale;		// Global movescale used to facilitate different moverates, // 1A
...
typedef struct usercmd_s
{
	byte	msec; //0x00
	byte	buttons; //0x01
	byte	lightlevel;		// light level the player is standing on 0x02
	char	lean;			// -1 or 1 0x03
	short	angles[3]; //0x04 0x006 0x08
	short	forwardmove; // 0xa
	short	sidemove; // 0xC
	short	upmove; // 0xE
	float	fireEvent; // 0x10
	float	altfireEvent; // 0x14
} usercmd_t;
...
typedef struct
{
	...
	// command (in)
	usercmd_t		cmd; // 1b
	qboolean		snapinitial;	// if s has been changed outside pmove // 33

	// results (out)
	int			numtouch; // 37
	struct edict_s	*touchents[MAXTOUCH]; // 3b

	vec3_t		viewangles;			// clamped // bb
	float		viewheight; // c7

	vec3_t		mins, maxs;			// bounding box size // cb

	struct edict_s	*groundentity; // e3
	int			watertype; // e7
	int			waterlevel; // eb

	// callbacks to test the world
	trace_t		(*trace) (vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end); // eF
	int			(*pointcontents) (vec3_t point); // f3
	int			(*checkOnOtherPlayer)(unsigned int groundEntity); // f7

	float		friction_loss; // fb
} pmove_t;
