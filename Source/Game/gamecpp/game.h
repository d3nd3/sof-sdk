...
typedef struct
{
	...
	int		(*soundindex) (const char *name); // 4
	int		(*effectindex) (const char *name); // 8
	int		(*imageindex) (const char *name); // 10

	void	(*unload_sound) (const char *name); // 14 
	qboolean (*FilterPacket) (char *from); // 18

	void	(*CreateGhoulConfigStrings) (void); // 1c

	void	(*setmodel) (edict_t *ent, char *name); // 20
	void	(*setrendermodel) (edict_t *ent, char *name); // 24

	// ClientCommand parameter access.

	int		(*argc) (void); // 28
	char	*(*argv) (int n); // 2c
	char	*(*args) (void);	// concatenation of all argv >= 1 30

	// Special messages.

	void	(*bprintf) (int printlevel, char *fmt, ...); // 34
	void	(*dprintf) (char *fmt, ...); // 38
	void	(*cprintf) (edict_t *ent, int printlevel, char *fmt, ...); // 3c
	void	(*clprintf) (edict_t *ent, edict_t *from, int color, char *fmt, ...); // 40
	void	(*welcomeprint) (edict_t *ent); // 44
...
typedef struct
{
	...
	qboolean	(*ClientConnect) (edict_t *ent, char *userinfo); // 20
	void		(*ClientBegin) (edict_t *ent); // 24
	void		(*ClientUserinfoChanged) (edict_t *ent, char *userinfo, bool not_first_time); // 28
	void		(*ClientDisconnect) (edict_t *ent); // 2c
	void		(*ClientCommand) (edict_t *ent); // 30
	void		(*ClientThink) (edict_t *ent, usercmd_t *cmd); // 34
	void		(*ResetCTFTeam) (edict_t *ent); // 38

	int			(*GameAllowASave) (void); // 3c
	void		(*SavesLeft) (void); // 40
	void		(*GetGameStats) (void); // 44
	void		(*UpdateInven) (void); // 48
	const char	*(*GetDMGameName) (void); // 4c
	byte		(*GetCinematicFreeze) (void); // 50
	void		(*SetCinematicFreeze) (byte cf); // 54

	float		(*RunFrame) (int serverframe); // 58
	...
	struct edict_s	*edicts; // 5c
	int			edict_size; // 60
	int			num_edicts;		// current number, <= max_edicts 64
	int			max_edicts; // 68
} game_export_t;
