...
typedef struct miptex_s
{
	int				version; // 0 
	char			name[32]; // 4
	unsigned		width[MIPLEVELS], height[MIPLEVELS]; // 24 44
	unsigned		offsets[MIPLEVELS];		// four mip maps stored // 64
	char			animname[32];			// next frame in animation chain // 84
	paletteRGB_t	palette[PAL_SIZE]; // A4
	int				flags; // 3A4
	int				contents;// 3A8
	int				value; // 3AC
} miptex_t;
...
{
	int			version;//0
	char		name[MAX_OSPATH];//4
	char		altname[MAX_OSPATH];			// texture substitution 128 0x84
	char		animname[MAX_OSPATH];			// next frame in animation chain //256 0x104
	char		damagename[MAX_OSPATH];		// image that should be shown when damaged // 384 0x184
	unsigned	width[MIPLEVELS], height[MIPLEVELS]; // 512 0x204
	unsigned	offsets[MIPLEVELS]; //640 0x284
	int			flags; //704 0x2C4
	int			contents; // 708 0x2C8
	int			value;// 712 0x2CC
	float		scale_x, scale_y; // 716 720 0x2D0 0x2D4
	int			mip_scale;
